...
\begin{itemize}
	...
	\item \emph{Adverbs} are annotated in a fashion closely related to that of adjectives. However the result might either by a \emph{change} or a \emph{scale}, a choice determined by the lemma: normally adverbs are annotated by the change in the same manner as adjectives, however \emph{intensifiers} and \emph{qualifiers}, i.e.\ adverbs that respectively strengthened or weakened the meaning, are scaled. Section~\ref{sec:sentimentAdverb} gives further details on how this choice is made. Finally special care are taken about negating adverbs, i.e. ``not'', which are scaled with a value $j = -1$.
...
The approach used to calculate sentiment polarity values for adjectives is very similar to the one presented by \citeauthor{valenceShifting} \shortcite{valenceShifting}, namely a domain specific set of positive and negative \emph{seed concepts} are identified, respectively $S_\mathrm{pos}$ and $S_\mathrm{neg}$. Each semantic concept in the sets can by described by a lexical unit and the specific index for that lexical unit that entail the desired concept, e.g.\ as shown in (\ref{eq:s_pos}) and (\ref{eq:s_neg}).
\begin{align}
...
Given an adjective with lemma $\ell \in \Sigma^\star$, the set of semantic concepts that this adjective may entail, $M(\ell)$ are considered.  Since there the system only has very limited domain knowledge, namely $S_\mathrm{pos}$ and $S_\mathrm{neg}$, there is no way of choosing which of the semantic concepts that is yielded by $M(\ell)$ is the ``right'' one. The solution is simply to take the top $n$ most \emph{frequent} usage of $\ell$, which off cause assumes that such total order on $M(\ell)$ is available.

Finally to calculate the sentiment polarity value for $\ell$ the graph $G_\mathrm{adj}$ is considered given by (\ref{eq:g_adj}), i.e.\ the graph given by both the \emph{similar}-relation and the \emph{see also}-relation, since both closely and coarsely related concepts are considered.
\begin{align}
	G_\mathrm{adj} = (S, r_\mathrm{similar} \cup r_\mathrm{see\text{-}also})
	\label{eq:g_adj}
\end{align}
 
The sentiment polarity value, $p_\mathrm{adj}(\ell)$, for the adjective with lemma $\ell$, is then given by the \emph{difference} of the sum of the \emph{normalized distances} in $G_\mathrm{adj}$, denoted $d_\mathrm{adj}$, to all negative and positive seed concepts for each of the top $n$ most \emph{frequent} usage of $\ell$, denoted by $M^n(\ell)$ cf.\ (\ref{eq:s_adj}). This follows the intuition, that positive adjectives are \emph{closer} to $S_\mathrm{pos}$ than $S_\mathrm{neg}$ and vice-verse.
\begin{align}
    p_\mathrm{adj}(\ell) = \left( \sum_{s \in M^n(\ell)}\sum_{s' \in S_\mathrm{neg}} d_\mathrm{adj}(s, s') \right) - \left( \sum_{s \in M^n(\ell)}\sum_{s' \in S_\mathrm{pos}} d_\mathrm{adj}(s, s') \right)
	\label{eq:s_adj}
\end{align}

Clearly this intuition is only valid if $|S_\mathrm{pos}| = |S_\mathrm{neg}|$, or more precisely, the probability of some random semantic concept is close to a positive concept should be the same as the probability of it being close to a negative concept. Furthermore it is assumed that $d_\mathrm{adj}$ is \emph{normalized} cf.\ (\ref{eq:d_adj}), i.e. it always yields a value, such that $p_\mathrm{adj}(\ell) \in \left[-\omega; \omega \right]$.
\begin{align}
    d_\mathrm{adj}(s, s') \in \left[\frac{-\omega}{n\cdot|S_\mathrm{pos}|}; \frac{\omega}{n \cdot |S_\mathrm{pos}|}\right]
    \label{eq:d_adj}
\end{align}

\section{Sentiment polarity of adverbs}
\label{sec:sentimentAdverb}
The approach for calculating sentiment polarity values for adverbs 
