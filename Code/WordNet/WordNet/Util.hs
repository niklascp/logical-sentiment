...
data IOModeEx = BinaryMode IOMode 
              | AsciiMode IOMode
...
tryMaybe a = (a >>= return . Just) `catch` (\e -> (const (return Nothing)) (e :: SomeException))

tryMaybeWarn :: (SomeException -> IO ()) -> IO a -> IO (Maybe a)
tryMaybeWarn warn a = (a >>= return . Just) `catch` (\e -> warn (e :: SomeException) >> return Nothing)
