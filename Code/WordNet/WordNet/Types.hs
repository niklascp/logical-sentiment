...
instance Show SearchResult where
  showsPrec i sr@(SearchResult { srSynset = ss }) =
    showString (unwords $ map (\(w,_, SenseNumber i) -> w ++ "#" ++ show i) $ ssWords ss)
