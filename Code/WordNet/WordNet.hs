...
searchByOverview overview pos sense = unsafePerformIO $ 
  case (case pos of { Noun -> T.nounIndex ; Verb -> T.verbIndex ; Adj -> T.adjIndex ; Adv -> T.advIndex })
    ...
    Just idx -> do
      ...
      -- putStr $ unlines $ map show synsets
...
-- closureOn :: (?wne :: WordNetEnv) => WN (Form -> SearchResult -> Tree SearchResult)
...
-- meet :: (?wne :: WordNetEnv) => Bag b (Tree SearchResult) => WN (b (Tree SearchResult) -> SearchResult -> SearchResult -> Maybe SearchResult)
...
-- -- lookup.
-- meetPaths :: (?wne :: WordNetEnv) => Bag b (Tree SearchResult) => 
--              WN (
