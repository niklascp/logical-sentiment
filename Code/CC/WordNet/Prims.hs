...
readSynset wne searchPos offset w = do
  ...
  let ss4 = ss3 -- if fp && searchPos == Adj && ssType ss3 == UnknownEPos
...
-- excLookup looks for a word in the exceptionlist for the given part of speech.
excLookup :: WordNetEnv -> String -> POS -> IO (Maybe String)
excLookup wne w pos = do
  ml <- binarySearch (excHandles wne ! pos) w
  case ml of
    Nothing -> return Nothing
    Just l -> do
      (_:baseForm:_) <- matchN 2 (words l)
      return (Just $ baseForm)
